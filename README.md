...
`assets/style.css` 顶部 `:root` 里改，例如 `--primary-color` 就是主色调。
...
- **选项超过 8 个会自动变成下拉框**（省地方）。想改这个阈值，动 `assets/app.js` 顶部的 `CAPSULE_LIMIT`；字段名超过 22 字会自动独占一行，对应 `LONG_LABEL`。
